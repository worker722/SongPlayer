            ...
            <Timeline ref={$blockRef}>
                <div ref={$subsRef}>
                    {currentSubs.map((sub, key) => {
                        var sub_text = sub.slices?.map?.(item => item.fragments?.map?.(tmp => tmp.text)?.join(""))?.join?.("")
